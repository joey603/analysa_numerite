'''
import numpy as np

from colors import bcolors
from matrix_utility import swap_rows_elementary_matrix, row_addition_elementary_matrix


def lu(A):
        N = len(A)
        L = np.eye(N) # Create an identity matrix of size N x N

        for i in range(N):

            # Partial Pivoting: Find the pivot row with the largest absolute value in the current column
            pivot_row = i
            v_max = A[pivot_row][i]
            for j in range(i + 1, N):
                if abs(A[j][i]) > v_max:
                    v_max = A[j][i]
                    pivot_row = j

            # if a principal diagonal element is zero,it denotes that matrix is singular,
            # and will lead to a division-by-zero later.
            if A[i][pivot_row] == 0:
                raise ValueError("can't perform LU Decomposition")

            # Swap the current row with the pivot row
            if pivot_row != i:
                e_matrix = swap_rows_elementary_matrix(N, i, pivot_row)
                print(f"elementary matrix for swap between row {i} to row {pivot_row} :\n {e_matrix} \n")
                A = np.dot(e_matrix, A)
                print(f"The matrix after elementary operation :\n {A}")
                print(bcolors.OKGREEN,"---------------------------------------------------------------------------", bcolors.ENDC)

            for j in range(i + 1, N):

                #  Compute the multiplier
                m = -A[j][i] / A[i][i]
                e_matrix = row_addition_elementary_matrix(N, j, i, m)
                e_inverse = np.linalg.inv(e_matrix)
                L = np.dot(L, e_inverse)
                A = np.dot(e_matrix, A)
                print(f"elementary matrix to zero the element in row {j} below the pivot in column {i} :\n {e_matrix} \n")
                print(f"The matrix after elementary operation :\n {A}")
                print(bcolors.OKGREEN,"---------------------------------------------------------------------------", bcolors.ENDC)

        U = A
        return L, U


# function to calculate the values of the unknowns
def backward_substitution(mat):
    N = len(mat)
    x = np.zeros(N)  # An array to store solution

    # Start calculating from last equation up to the first
    for i in range(N - 1, -1, -1):

        x[i] = mat[i][N]

        # Initialize j to i+1 since matrix is upper triangular
        for j in range(i + 1, N):
            x[i] -= mat[i][j] * x[j]

        x[i] = (x[i] / mat[i][i])

    return x

def lu_solve(A_b):
    L, U = lu(A_b)
    print("Lower triangular matrix L:\n", L)
    print("Upper triangular matrix U:\n", U)

    result = backward_substitution(U)
    print(bcolors.OKBLUE,"\nSolution for the system:")
    for x in result:
        print("{:.6f}".format(x))


if __name__ == '__main__':


    A_b = [[1, -1, 2, -1, -8],
        [2, -2, 3, -3, -20],
        [1, 1, 1, 0, -2],
        [1, -1, 4, 3, 4]]


    lu_solve(A_b)
'''



'''  AFTER REFACTOR'''



import numpy as np
from colors import bcolors
from matrix_utility import swap_rows_elementary_matrix, row_addition_elementary_matrix


class LUDecomposer:
    """Handles LU decomposition with elementary matrix transformations."""

    def __init__(self, A):
        self.A = np.array(A, dtype=np.double)
        self.N = len(self.A)
        self.L = np.eye(self.N)  # Initialize L as an identity matrix

    def decompose(self):
        """Performs LU decomposition using elementary row operations."""
        for i in range(self.N):
            # Partial Pivoting: Find the row with the largest absolute value in the current column
            pivot_row = max(range(i, self.N), key=lambda r: abs(self.A[r][i]))

            # Check for singular matrix
            if self.A[pivot_row][i] == 0:
                raise ValueError("LU decomposition cannot be performed (singular matrix)")

            # Swap rows if needed
            if pivot_row != i:
                e_matrix = swap_rows_elementary_matrix(self.N, i, pivot_row)
                self.A = np.dot(e_matrix, self.A)

            # Perform elimination to form U
            for j in range(i + 1, self.N):
                m = -self.A[j][i] / self.A[i][i]  # Compute multiplier
                e_matrix = row_addition_elementary_matrix(self.N, j, i, m)
                e_inverse = np.linalg.inv(e_matrix)
                self.L = np.dot(self.L, e_inverse)
                self.A = np.dot(e_matrix, self.A)

        U = self.A
        return self.L, U


class ForwardSubstitutionSolver:
    """Solves a lower triangular system using forward substitution."""

    @staticmethod
    def solve(L, b):
        """Solves Lz = b using forward substitution."""
        N = len(L)
        z = np.zeros(N)

        for i in range(N):
            z[i] = b[i] - np.dot(L[i, :i], z[:i])
            z[i] /= L[i, i]

        return z


class BackwardSubstitutionSolver:
    """Solves an upper triangular system using backward substitution."""

    @staticmethod
    def solve(U, z):
        """Solves Ux = z using backward substitution."""
        N = len(U)
        x = np.zeros(N)

        for i in range(N - 1, -1, -1):
            x[i] = z[i] - np.dot(U[i, i+1:], x[i+1:])
            x[i] /= U[i, i]

        return x


class LUSolver:
    """Combines LU decomposition and forward/backward substitution to solve Ax = b."""

    def __init__(self, A, b):
        self.A = np.array(A, dtype=np.double)
        self.b = np.array(b, dtype=np.double)

    def solve(self):
        """Performs LU decomposition and solves the system."""
        decomposer = LUDecomposer(self.A)
        L, U = decomposer.decompose()

        print("\nLower triangular matrix L:\n", L)
        print("Upper triangular matrix U:\n", U)

        # Solve Lz = b using forward substitution
        forward_solver = ForwardSubstitutionSolver()
        z = forward_solver.solve(L, self.b)

        # Solve Ux = z using backward substitution
        backward_solver = BackwardSubstitutionSolver()
        solution = backward_solver.solve(U, z)

        print(bcolors.OKBLUE, "\nSolution for the system:")
        for x in solution:
            print(f"{x:.6f}")

        return solution


if __name__ == '__main__':
    #  Problem 2: System Ax = b from the PDF
    A = np.array([[-1, -1, 2], [2, -1, 1], [2, 2, 2]], dtype=np.double)
    b = np.array([-5, -1, 4], dtype=np.double)

    print("\n Solving Problem 2 using LU Factorization...")

    #  Use LU Solver
    solver = LUSolver(A, b)
    solution = solver.solve()

    #  Extract and print value of 'c'
    c_value = solution[2]

    print(bcolors.OKBLUE, f"\n Approximate value of c: {c_value}", bcolors.ENDC)
